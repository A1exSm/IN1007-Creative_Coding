...
void setup() {
  ...
  ArrayList<Song> japanese = new ArrayList<>();
  ArrayList<Song> english = new ArrayList<>();
  ArrayList<Song> german = new ArrayList<>();
  ArrayList<Song> finnish = new ArrayList<>();
  for (TableRow row : table.rows()) {
    ...
    float durationSeconds = (((temp[0]*10 + temp[1])*60)+((temp[3]*10)+temp[4])-32000)/2;
...
class Category {
  ...
  Category(Song[] songs, String category) {
    for (Song song : songs) {
      if (song.language.equals(category)){}
    }
  }
}
